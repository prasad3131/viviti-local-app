...
import { getFacePhotos, FaceCluster, FacePhoto } from '../lib/api';
...
export default function FacePhotosScreen({ face, onBack, onOpenPhoto }: Props) {
  const [photos, setPhotos] = useState<FacePhoto[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    getFacePhotos(face.id)
      .then(setPhotos)
      .catch(() => {})
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          <Text style={styles.title}>{face.name ?? 'Unknown'}</Text>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          numColumns={COL}
          ListEmptyComponent={<Text style={styles.empty}>No photos found for this person.</Text>}
          renderItem={({ item }) => (
            <TouchableOpacity
